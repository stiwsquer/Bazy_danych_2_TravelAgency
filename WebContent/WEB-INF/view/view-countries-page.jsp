...
<body>
	...
	<div id = "wrapper">
		<div id="header">
			<h2>Travel Agency - View Countries</h2>
		</div>
	</div>
	...
	<div id="container">
		<div id="content">
			...
			<input type="button" value="Add Country" 
				   onclick="window.location.href='addCountryForm'; return false;" 
				   class = "add-button"
			/> 
			...
			<table>
				...
				<c:forEach var="tempCountry" items="${countries}">
					...
					<c:url var="updateLink" value="/user/updateCountryForm">
						<c:param name="countryId" value="${tempCountry.countryId}" />
					</c:url>
					
					<!-- construct an "delete" link with country id -->
					<c:url var="deleteLink" value="/user/deleteCountry">
						<c:param name="countryId" value="${tempCountry.countryId}" />
					</c:url>
